...
json_path="/iperf/$BITRATE-P$PARALLEL-T70s-TCP-DOWNLOAD-PACKETLOSS-$TESTCASE-$time.json"

# TCP DOWNLOAD test with regularly changing packetloss

echo "TCP Download test with changing packet loss | Bitrate: $BITRATE"

iperf3 -c $IPERF_SERVER_IP --port 7070 -t 70 -b "$BITRATE"M -P "$PARALLEL" -O 2 --reverse --get-server-output --json --logfile "$json_path" &
